...
export const getPageTitle = (asPath, locale) => {

  const categoryPath = asPath.split('?')[0]

  if (categoryPath == '/category/quilt%20cover') {
    return locale === 'ar-DZ' ? 'أغلفة اللحاف' : locale === 'fr-FR' ? 'Housses de couette' : 'Quilt covers'
  }
  else if (categoryPath == '/category/bed%20sheet') {
    return locale === 'ar-DZ' ? 'شراشف' : locale === 'fr-FR' ? 'Draps de lit' : 'Bed sheets'
  }
  else if (categoryPath == '/category/pillowcase') {
